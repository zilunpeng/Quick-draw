...
class build_feature():
    def __init__(self, writing_path):
    ...
    def get_tt_ind(x, y, x_ngbr, y_ngbr, is_ngbr_center, size, self):
        valid_ngbr = self.get_valid_coord(x_ngbr, y_ngbr, size)
        true_ngbr = self.intersect_coords(x, y, x_ngbr, y_ngbr)
        inds = np.logical_and(true_ngbr, valid_ngbr)
        inds = self.sub2ind(x_ngbr[inds], y_ngbr[inds], size) if is_ngbr_center else self.sub2ind(x[inds], y[inds], size)
        return inds, true_ngbr, valid_ngbr

    def get_tf_ft_ind(x, y, x_ngbr, y_ngbr, true_ngbr, valid_ngbr, is_ngbr_center, size, self):
        false_ngbr = np.logical_not(true_ngbr)
        inds = np.logical_and(false_ngbr, valid_ngbr)
        inds = self.sub2ind(x_ngbr[inds], y_ngbr[inds], size) if is_ngbr_center else self.sub2ind(x[inds], y[inds], size)
        return inds

    def set_edge_feature(x, y, get_center_ngbr_coords, get_noncent_ngbr_coords, size, self):
        tot_nodes = size * size
        ...
        inds, true_ngbr, valid_ngbr = self.get_tt_ind(x, y, x_cent, y_cent, is_ngbr_center=True, size=size)
        edge_feature_tt[inds] = 1
        edge_feature_ft[self.get_tf_ft_ind(x, y, x_cent, y_cent, true_ngbr, valid_ngbr, is_ngbr_center=True, size=size)] = 1

        inds, true_ngbr, valid_ngbr = self.get_tt_ind(x, y, x_noncent, y_noncent, is_ngbr_center=False, size=size)
        edge_feature_tt[inds] = 1
        edge_feature_tf[self.get_tf_ft_ind(x, y, x_noncent, y_noncent, true_ngbr, valid_ngbr, is_ngbr_center=False, size=size)] = 1

        return np.concatenate((edge_feature_tt, edge_feature_tf, edge_feature_ft))

    def set_feature_mat(drawing, self):
        size = 256
        feature_node = np.zeros(size * size)
        (x, y) = self.find_row_col_inds(drawing)
        feature_node[self.sub2ind(x, y, size)] = 1
        feature_h_edge = self.set_edge_feature(x, y, self.get_left_node_coords, self.get_right_node_coords, size)
        feature_v_edge = self.set_edge_feature(x, y, self.get_up_node_coords, self.get_down_node_coords, size)
        feature_nw_edge = self.set_edge_feature(x, y, self.get_up_left_node_coords, self.get_down_right_node_coords, size)
        feature_ne_edge = self.set_edge_feature(x, y, self.get_up_right_node_coords, self.get_down_left_node_coords, size)
        feature = np.where(np.concatenate((feature_node, feature_h_edge, feature_v_edge, feature_nw_edge, feature_ne_edge)))
        np.savetxt(self.writing_path, feature)
